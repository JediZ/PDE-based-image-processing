...
param.AA = [];			%Allowed area for segmentation (1=allowed, 0=not allowed). Defaults to 1 for the whole domain.
param.srem_thr = 0.002;		%Segment size remove threshold. Segments smaller than this will be removed.
param.polyorder = 1;		%Order of the polynomial used for modeling the surface (1 or 2).
param.seeds = 15;		%Number of seeds.
param.scl_factor = 0.7;		%Downscale factor
param.gen_scl = 0.2;		%Downscaled size (percentage) of the orignal image when generating seeds
param.rc_scl = 0.4;		%Downscaled size (percentage) of the original image for region competition
...
SEG = [];
D{1} = double(Din);
D{1}(isnan(D{1})) = 0;		%We don't like NaNs
...
for scl=2:scales
	D{scl} = imresize( D{scl-1}, param.scl_factor );
...
%
%OUTPUT
%PHIout			=		Level-set function(s), one per segment.
%SParam			=		Surface parameters.
		...
		for iter=1:iterations
			...
			switch polyorder
				case 1
					...
					distD = ( H1eq(1)*X + H1eq(2)*Y + H1eq(3) - Din{scl} ).^2;
				case 2
					...
					%Distance to the fitted surface
					distD = ( H1eq(1)*X2 + H1eq(2)*Y2 + H1eq(3)*XY + H1eq(4)*X + H1eq(5)*Y + H1eq(6) - Din{scl} ).^2;
				otherwise
				  error('Only polynomials of 1st and 2nd order have been implemented')
			end

			H1sum = sum(sum(H1));
			covect = [sum(distD(H1))/H1sum];
...
for cscl=1:numel(pyramid)-1
	...
	for iter=1:iterations
		...
		if mod(iter, 2) || SIG_recalc==true
			...
			%DH = 1./(0.5*pi*(1+ (PHI{scl}/0.5).^2) );	%e = 0.5
			DH = 1./(pi*(1+(PHI{scl}.^2)./1));		%e = 1.0
			DH( DH<0.06 ) = 0.06;				%To keep things 'active'
			...
			for s=1:segments
				...
				case 1
					...
					distD = ((surface1(1,s)*X + surface1(2,s)*Y + surface1(3,s)) - Din{scl}).^2;
				case 2
					%Quadratic surface
					...
					[surface1(:,s) Err] = SurfaceEquation(	single( [Xs.^2 Ys.^2 Xs.*Ys Xs Ys ones(numel(Xs),1)] ), ...
										...
										single( 10 ) );
					%Distance from the plane
					distD = (surface1(1,s)*XX + surface1(2,s)*YY + surface1(3,s)*XY + surface1(4,s)*X + surface1(5,s)*Y + surface1(6,s) - Din{scl}).^2;
				otherwise
					error('Only polynomials of 1st and 2nd order have been implemented')
			    end
