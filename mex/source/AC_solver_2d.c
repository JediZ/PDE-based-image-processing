/*
function [PHI_out varargout] = ac_solver_2D(PHIin, D, GradNorm, Diff, tau, nu)

AOS solver for active contour model.

A MEX c-program for Matlab.

INPUT
PHIin		=		Level set function O.
D		=		Data term.
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs,const mxArray *prhs[])
{
	...
	struct matrixM Diff_in = {0,NULL,NULL,0,0,0,0};
	...
	*/
	...
	if (nrhs != 6) {
		mexErrMsgTxt("AC_solver_2D parameter error: wrong number of input parameters!");
	} 
	/*
	//------------
	//--- PHI in ---
	...
	*/
	...
	if (!mxIsSingle(prhs[0])){
		mexErrMsgTxt("AC_solver_2D error: 'PHI_in' must be a noncomplex single-valued matrix.");
	}
	...
	*/
	...
	if (!mxIsSingle(prhs[1])){
		mexErrMsgTxt("AC_solver_2D error: 'D_in' must be a noncomplex single-valued matrix.");
	}
	/*Get number of dimensions and elements per dimension */
	...
	*/
	...
	if (!mxIsSingle(prhs[2])){
		mexErrMsgTxt("AC_solver_2D error: 'GradNorm_in' must be a noncomplex single-valued matrix.");
	}
	/*Get number of dimensions and elements per dimension */
	GradNorm_in.ndims = mxGetNumberOfDimensions(prhs[2]);
	GradNorm_in.dimElems = mxGetDimensions(prhs[2]);
	GradNorm_in.data = (float*)mxGetPr( prhs[2] );
	/*
	//------------
	//--- DH in ---
	//------------
	*/
	...
	if (!mxIsSingle(prhs[3])){
		mexErrMsgTxt("AC_solver_2D error: 'Diff_in' must be a noncomplex single-valued matrix.");
	}
	/*Get number of dimensions and elements per dimension */
	Diff_in.ndims = mxGetNumberOfDimensions(prhs[3]);
	Diff_in.dimElems = mxGetDimensions(prhs[3]);
	Diff_in.data = (float*)mxGetPr(prhs[3]);
	...
	*/
	...
	if (!mxIsSingle(prhs[4])){
		mexErrMsgTxt("AC_solver_2D error: 'tau' must be a noncomplex, single-type scalar");
	}
	...
	*/
	...
	if (!mxIsSingle(prhs[5])){
		mexErrMsgTxt("AC_solver_2D error: 'nu' must be a noncomplex, single-type scalar");
	}
	...
	switch( PHI_in.ndims )
	{
	  case 1:
		if (PHI_in.dimElems[0]>MAX_BUF_SIZE)
		    mexErrMsgTxt("AC_solver_2D error: MAX_BUF_SIZE too small...increase the size in the header file!");
		break;
	  default :
	...
	*/
	...
	if( nlhs<1 ){
		mexErrMsgTxt("ac_solver_2D error insufficient number of outputs. Outputs from this function is 'PHI_out'");
	}else{
	
		if( (plhs[0] = mxCreateNumericArray(PHI_out.ndims,PHI_out.dimElems, mxSINGLE_CLASS, mxREAL))==NULL )
			mexErrMsgTxt("ac_solver_2D error: error reserving space for output variable 'PHI_out'");

		if( (PHI_out.data = (float *)mxGetPr( plhs[0] ))==NULL )
			mexErrMsgTxt("ac_solver_2D error: error obtaining pointer to output variable 'PHI_out'");
	}
